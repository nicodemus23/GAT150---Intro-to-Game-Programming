...
namespace kiko
{
	...
	void Emitter::Emit()
	{
		...
		if (particle)
		{
			...
			data.color = Color
			{
				static_cast<float>(random(5)), // Random red component (0-255)
				static_cast<float>(random(5)), // Random green component (0-255)
				static_cast<float>(random(5)), // Random blue component (0-255)
				static_cast<float>(random(10)) // Alpha value set to 255 for fully opaque particles (you can adjust as needed)
			};
