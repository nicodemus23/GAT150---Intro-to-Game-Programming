...
namespace kiko
{
	...
	void SpriteAnimRenderComponent::Update(float dt)
	{
		...
		if (frameTimer <= 0)
		{
			...
			frame++; // we're at next frame now
			if (frame > m_sequence->endFrame)
			{
				frame = (m_sequence->loop) ? m_sequence->startFrame : m_sequence->endFrame; // if looping, we're at first frame.. if not, we're at the end
			}
		}
		UpdateSource();
	}
	...
	void SpriteAnimRenderComponent::SetSequence(const std::string& name)
	{
		...
		if (m_sequence && m_sequence->name == name) return; // sequence-> is the current sequence that we're in 
	...
	void SpriteAnimRenderComponent::UpdateSource()
	{
		...
		// set source rect form current column/row size 
